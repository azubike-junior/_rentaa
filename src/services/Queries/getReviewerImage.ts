...
export const getReviewerImage = createAsyncThunk(
  "getReviewerImage",
...
const getReviewerImageSlice = createSlice({
  name: "bvn",
  initialState,
  reducers: {},
  extraReducers: (builder) => {
    builder.addCase(getReviewerImage.rejected, (state, action) => {
      state.error = action.payload;
      state.error2 = action.error.name;
      state.loading = false;
      state.isSuccessful = false;
    });
    builder.addCase(getReviewerImage.fulfilled, (state, action) => {
      state.loading = true;
    ...
    builder.addCase(getReviewerImage.pending, (state, action) => {
      state.loading = true;
      state.error = action.payload;
    });
  },
...
export default getReviewerImageSlice.reducer;
